...
from stride.xai.recurrence.protree.explainers import TExplainer, APete
...
class RaceP(DriftDetector):
    ...
    def _compute_metric(self) -> float | dict[int | str, float]:
        if self.measure in ["mutual_information", "rand_index", "completeness", "fowlkes_mallows"]:
            import stride.xai.recurrence.protree.metrics.compare

            self._direction = "decrease"

            metric = getattr(stride.xai.recurrence.protree.metrics.compare, self.measure)
            return self._compute_cluster_metric(metric)

        elif self.measure == "prototype_reassignment_impact":
            from stride.xai.recurrence.protree.metrics.compare import prototype_reassignment_impact
            ...
            if self.strategy == "class":
                from stride.xai.recurrence.protree.metrics.compare import classwise_mean_minimal_distance

                return self._compute_spatial_metric(classwise_mean_minimal_distance)

            elif self.strategy == "total":
                from stride.xai.recurrence.protree.metrics.compare import mean_minimal_distance
            ...
            if self.strategy == "class":
                from stride.xai.recurrence.protree.metrics.compare import centroids_displacements

                return self._compute_spatial_metric(centroids_displacements)

            elif self.strategy == "total":
                from stride.xai.recurrence.protree.metrics.compare import mean_centroid_displacement
