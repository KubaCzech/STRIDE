...
from stride.xai.recurrence.protree import TPrototypes, TTarget, TDataBatch
from stride.xai.recurrence.protree.explainers.tree_distance import IExplainer
from stride.xai.recurrence.protree.utils import (
    get_x_belonging_to_cls,
    get_re_idx,
    flatten_prototypes,
    get_x_not_belonging_to_cls,
)


def individual_contribution(prototypes: TPrototypes, cls: str | int, idx: int, explainer: IExplainer, x: TDataBatch) -> float:
    from stride.xai.recurrence.protree.metrics.group import fidelity_with_model
